    ...
    <div className="grid grid-cols-3 gap-2.5 sm:gap-4 sm:p-4">
      {gridData?.map((sin, idx) => {
        ...
        return (
          <div
            key={idx}
            className="flex relative overflow-hidden group shadow-lg shadow-sky-900/20 items-center justify-center space-x-2 flex-col gap-4 bg-indigo-950/40 p-4 rounded-lg hover:-translate-y-1.5 ease-linear transition-transform delay-300"
          >
            <Icon className={`h-6 w-6 ${sin?.textc} `} />
            <span className="text-white text-[12px] sm:text-sm md:text-[16px]">
              {sin?.name}
            </span>
            <div
              className="shine absolute top-0 left-[-150%] w-[150%] h-full 
