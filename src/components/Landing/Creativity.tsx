  ...
  return (
    <section className="bg- py-20 text-white">
      <div className=" mx-auto lg:px-6">
        <SectionHeader title="Some Creative Design." 
        ...
        <div className="w-full h-[420px] md:h-[540px] shadow-2xl shadow-indigo-900/20 overflow-hidden border border-indigo-900/10 rounded-2xl">
          <ScrollStack className="p-0! w-full!">
            {
              items?.map((item,idx)=>(
                <ScrollStackItem key={idx} itemClassName={`border ${item?.bgColor} p-2! smm:p-4! rounded-[12px]! lg:rounded-[42px]!`}>
                  <div className=" w-[90%]! md:w-[80%] xl:w-full sm:gap-5 flex flex-col mx-auto md:flex-row gap-2 lg:gap-5! sm:gap-  md:p-5 md:items-center justify-center lg:justify-evenly">  
                    <Image src={item?.img} className="rounded-lg sm:w-[85%] mx-auto md:w-[80%] lg:w-[100%]" height={300} width={500} alt="shoplyyy"  />
                    <div className="w-full!">
                      <h2 className="text-[16px] smm:text-[17px] md:text-xl lg:text-3xl font-bold">{item?.title}</h2>
                      <p className="font-sans text-xs md:text-[15px] lg:tracking-wider">{item?.description}</p>
                    </div>
                  </div>
                </ScrollStackItem>
              ))
